...
const UserConfirmation = ({ socket }) => {
  ...
  if (jobDetails?.trackerStage === 'en route') return (
    <>
      ...
      >
        <Marker longitude={jobDetails.fixerLocation[0]} latitude={jobDetails.fixerLocation[1]} anchor='bottom' color='#c70a0a'/> {/* can add image of vehicle to Marker*/}
      </Map>
  ...
  return (
    <div>UserConfirmation</div> // default return? would indicate some type of issue so could be a fallback to cancel the Request or similar
  )
}
